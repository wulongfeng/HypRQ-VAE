...
from hyp_trainer import  Hyp_Trainer
...
def parse_args():
    ...
    parser.add_argument('--learner', type=str, default="Adam", help='optimizer')
    parser.add_argument('--lr_scheduler_type', type=str, default="other", help='scheduler')
    parser.add_argument('--lr_reduce_freq', type=str, default=None, help='reduce lr every lr-reduce-freq or None to keep lr constant')
    parser.add_argument('--gamma', type=float, default=0.5, help='gamma for lr scheduler')
    parser.add_argument('--grad_clip', type=float, default=1.0, help='max norm for gradient clipping, or None for no gradient clipping')

    parser.add_argument('--warmup_epochs', type=int, default=50, help='warmup epochs')
    parser.add_argument("--data_path", type=str,
                        default="../data/Instruments/Instruments.emb-llama-td.npy",
                        help="Input data path.")
    parser.add_argument("--log_path", type=str,
                            default="./hyp_rqvae.log",
                            help="log file path.")
    ...
    parser.add_argument("--use_sk", action='store_true', help="using sinkhorn algorithm or not")
...
if __name__ == '__main__':
    ...
    print(model)
    print(f'using sk or not from main func {args.use_sk}, initialize with kmeans or not:{args.kmeans_init}')
    data_loader = DataLoader(data,num_workers=args.num_workers,
                             batch_size=args.batch_size, shuffle=True,
                             pin_memory=True)
    trainer = Hyp_Trainer(args, model, len(data_loader))
