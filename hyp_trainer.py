...
class Hyp_Trainer(object):
    def __init__(self, args, model, data_num):
        ...
        self.lr_reduce_freq = args.lr_reduce_freq
    ...
    def _get_scheduler(self):
        ...
        if self.lr_scheduler_type.lower() == "linear":
            ...
            lr_scheduler = get_constant_schedule_with_warmup(optimizer=self.optimizer,
             ...
             lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer=self.optimizer,
                                                               step_size=int(
                                                                   self.lr_reduce_freq),
    ...
    def _generate_train_loss_output(self, epoch_idx, s_time, e_time, loss, recon_loss):
        ...
        train_loss_output_simplify = 'Training epoch {} \t[time cost:{:.2f} \t train loss: {:.4f}, reconstruction loss:{:.4f}]'.format(epoch_idx, e_time - s_time, loss, recon_loss)
        return train_loss_output + "]", train_loss_output_simplify
    ...
    def fit(self, data):
        ...
        for epoch_idx in range(self.epochs):
            ...
            #if epoch_idx % 50 == 0:
            train_loss_output, train_loss_output_simplify = self._generate_train_loss_output(
                epoch_idx, training_start_time, training_end_time, train_loss, train_recon_loss
            )
            self.logger.info(train_loss_output_simplify)
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                valid_score_output_simplify = 'Evaluation epoch {} \t[time cost:{:.2f} \t collision_rate: {}]'.format(epoch_idx, valid_end_time - valid_start_time, collision_rate)

                self.logger.info(valid_score_output_simplify)
